...
const getProviderById = async (req, res) => {
  try {
    const provider = await Provider.findById(req.params.providerId).populate('userId', 'name email phone location profileImage');
    if (!provider || !provider.approved) {
      return res.status(404).json({ message: 'Provider not found' });
    }
    const Review = require('../models/Review');
    const reviews = await Review.find({ providerId: provider._id }).populate('userId', 'name profileImage').sort({ createdAt: -1 }).limit(10);
